...
import { findGroup, runForGroup } from "../lib/academy";
import { buildGroupReport } from "../lib/academy/academyReports";

export default function AcademyGroup() {
  const { classId, groupId } = useParams();
  const { academyState, isRunning, error, loadClassState, runGroupCycle } = useAcademyContext();
  ...
  const group = findGroup(academyState, groupId);
  const run = runForGroup(academyState, groupId);
  const report = buildGroupReport(group || { id: groupId, name: "Groupe" }, run);

  const handlePlayCycle = () => {
    runGroupCycle(classId, groupId, {}).catch(() => undefined);
  };
    ...
    <div className="flex flex-col gap-6">
      <header className="page-header">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{report.groupName}</h1>
      ...
      {report.status === "not_started" ? (
        <div className="rounded-lg border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">
          Aucun scénario n'a encore été assigné à ce groupe.
        </div>
      ) : (
        <>
          <section aria-labelledby="group-kpis" className="flex flex-col gap-3">
            <h2 id="group-kpis" className="text-base font-semibold text-slate-900">Scoring</h2>
          ...
          <section aria-labelledby="group-objectives" className="flex flex-col gap-3">
            <h2 id="group-objectives" className="text-base font-semibold text-slate-900">Objectifs</h2>
            <Card>
          ...
          <section aria-labelledby="group-daily-reports" className="flex flex-col gap-3">
            <h2 id="group-daily-reports" className="text-base font-semibold text-slate-900">Rapports journaliers</h2>
            <Card>
