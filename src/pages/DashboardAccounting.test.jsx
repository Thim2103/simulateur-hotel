...
describe("Dashboard / accounting desk", () => {
  it("offers the balance sheet among the quick actions", () => {
    mount();
    expect(within(screen.getByTestId("quick-actions")).getByTestId("quick-accounting")).toHaveTextContent("Bilan comptable");
  });

  it("opens the desk from there", () => {
    mount();
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    fireEvent.click(screen.getByTestId("quick-accounting"));
    expect(screen.getByRole("dialog", { name: /bilan comptable/i })).toBeInTheDocument();
    expect(within(screen.getByRole("dialog")).getByTestId("accounting-panel")).toBeInTheDocument();
  });

  it("opens it from a #accounting link (the top-bar's Finance menu)", () => {
    mount(rich(), { hash: "#accounting" });
    expect(screen.getByRole("dialog", { name: /bilan comptable/i })).toBeInTheDocument();
  });
