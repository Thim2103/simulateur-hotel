// Integration test: Analytics + Housekeeping --
// housekeepingDiagnosticsToAnalytics() output merges cleanly into a real
// analyzeRun() Analysis object's own diagnostics list (see the Refonte
// Housekeeping request's section 5: "intégrer les diagnostics HK dans
// analyticsEngine").
...
import { runHousekeepingCycle, generateHousekeepingReport, housekeepingDiagnosticsToAnalytics } from "./housekeepingEngine";

const REFERENCE_DATE = new Date("2026-09-10T12:00:00Z");

test("housekeepingDiagnosticsToAnalytics() output can be concatenated into a real Analysis object's diagnostics", async () => {
  ...
  const hkState = runHousekeepingCycle({ hotelBundle: playedCareer.hotel, referenceDate: REFERENCE_DATE });
  const hkDiagnostics = generateHousekeepingReport(hkState).diagnostics;
  const adapted = housekeepingDiagnosticsToAnalytics(hkDiagnostics);

  const combined = [...analysis.diagnostics, ...adapted];

  // Every entry (Analytics' own, and the adapted Housekeeping ones)
  // shares the exact same {type, severity, message, cycleIndex} shape.
