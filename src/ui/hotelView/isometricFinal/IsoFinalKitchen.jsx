...
import { KITCHEN_PROPS } from "./IsoFinalSprites";

// Ground-floor room: cuisine -- fourneaux, frigo, plan de travail (see
// IsoFinalSprites.js's KITCHEN_PROPS).
export default function IsoFinalKitchen({ col, row }) {
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 text-center" style={{ left: x, top: y }}>
      <div
        ...
        style={{ backgroundColor: `${PALETTE.warmGrey}66`, border: `${STROKE_WIDTH} solid ${PALETTE.nightBlue}`, filter: SHADOW_FILTER }}
      >
        {KITCHEN_PROPS.map((prop, index) => (
          <span key={index} aria-hidden="true">{prop}</span>
        ))}
      </div>
      <span className="mt-1 block text-[10px] font-bold" style={{ color: PALETTE.nightBlue }}>Cuisine</span>
    </div>
  );
}
