...
export function QuickActions({ onOpenGrowth, reviewsToAnswer = 0, onOpenLoyalty, onOpenBanking, onOpenProjects, onOpenSuppliers, onOpenAccounting, onOpenTfeFeasibility }) {
  return (
    <section aria-label="Accès rapide" data-testid="quick-actions" className="flex flex-wrap items-center gap-2">
      ...
      {onOpenBanking && (
        <SoftButton tone="success" icon="🏦" data-testid="quick-banking" onClick={onOpenBanking}>
          Banque & Emprunts
        </SoftButton>
      )}
      {onOpenLoyalty && (
      ...
      {onOpenSuppliers && (
        <SoftButton tone="action" icon="📦" data-testid="quick-suppliers" onClick={onOpenSuppliers}>
          Fournisseurs
        </SoftButton>
      )}
      {onOpenAccounting && (
        <SoftButton tone="mice" icon="📊" data-testid="quick-accounting" onClick={onOpenAccounting}>
          Bilan comptable
        </SoftButton>
      )}
      {onOpenTfeFeasibility && (
        <SoftButton tone="vip" icon="🎓" data-testid="quick-tfe-feasibility" onClick={onOpenTfeFeasibility}>
          Plan de faisabilité
        </SoftButton>
      )}
    </section>
